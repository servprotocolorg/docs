...
```bash
servnode start --rpc.laddr="tcp://127.0.0.1:26657"
```
...
looking for. For example, a an Ethereum transaction on SERV (`MsgEthereumTx`) triggers an `event` of type `ethermint` and
...
Check the `Events` page to obtain the event list of each supported module on SERV.
:::
