...
Connect your Metamask wallet with SERV. 
...
:::note
If you are planning on developing on SERV locally and you haven’t already set up your own local node, refer to [the quickstart tutorial](./../../protocol/evmos-cli/single-node), or follow the instructions in the [GitHub repository](https://github.com/servprotocolorg/serv/).
:::

:::tip
If you need to add SERV Testnet and Mainnet to Metamask wallet, head over [here](https://chainlist.org/?testnets=true&search=serv) to add configurations.
:::
...
3. Click the `Connect Wallet` button under `SERV Testnet`
...
```bash
servnode keys unsafe-export-eth-key dev0
```
...
## Registering for SERV Domains

SERV Domains are modelled after the Ethereum Name Service (ENS), link [here](https://ens.domains/), SERV users can register for an SERV Domains with the help of [this](https://evmos.domains/) service. This service will use Metamask to connect and allow users to search for a name not taken. The registration process will take some SERV token.
