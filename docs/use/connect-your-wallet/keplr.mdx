...
Connect your Keplr wallet with SERV.
...
3. If you have not used Keplr before, set a **strong** password for the Keplr extension, and click **Confirm**
4. Switch to the Ethereum app on the Ledger, then select “SERV” from the Keplr chain registry to connect the public key
...
3. Make sure you are on the `SERV Testnet` network from the dropdown. Testnet networks will likely be towards the bottom
of the list.
4. Update either the `RPC` or `Rest` or both inputs and click `Confirm`

## Connect Keplr to Mainnet

Once you are signed in to the Keplr extension, you can connect the wallet with the SERV network. The SERV mainnet network
is already built into Keplr and look for the `SERV` network by clicking on the extension and scrolling to select the network.

## Connect to SERV Testnet
...
EVM-based chains. Since SERV supports EVM and Cosmos-based actions, both ENS and ICNS are supported. Users can have
