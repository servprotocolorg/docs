...
| -------------- | --------------- | ------------------------------------------------------------------ | ------------------------------------------------------------ | ------- |
| `serv_53970-2` | SERV Mainnet 2 | [SERV](https://github.com/evmos/mainnet/tree/main/serv_53970-2) | [<Highlighter keyword="mainnet_version" />](https://github.com/servprotocolorg/serv/releases) | `Live`  |
| `serv_53970-1` | SERV Mainnet 1 | [SERV](https://github.com/evmos/mainnet/tree/main/serv_53970-1) | [`v2.0.1`](https://github.com/servprotocolorg/serv/releases/v2.0.1) | `Stale` |
...
```bash
servnode config chain-id serv_53970-2
```
...
```bash
servnode init <your_custom_moniker> --chain-id serv_53970-2
```
...
By default, the `init` command creates your `~/.serv` (i.e `$HOME`) directory with subfolders `config/` and `data/`.
...
Download the `genesis.json` file from the [`archive`](https://rpc.serv.services/genesis) and copy it over
to the `config` directory: `~/.serv/config/genesis.json`. This is a genesis file with the chain-id and genesis
accounts balances.

```bash
wget https://rpc.serv.services/genesis
mv genesis.json ~/.serv/config/
```
...
to `$HOME/.serv/config/config.toml`. 
The [`mainnet`](https://github.com/servprotocolorg/serv) repo contains links to some
seed nodes.

Edit the file located in `~/.serv/config/config.toml` and the `seeds` to the following:
...
```bash
SEEDS=`curl -sL https://github.com/servprotocolorg/serv/releases/download/v12.2.3/seeds.txt | awk '{print $1}' | paste -s -d, -`
sed -i.bak -e "s/^seeds =.*/seeds = \"$SEEDS\"/" ~/.serv/config/config.toml
```
...
field in `~/.serv/config/config.toml` to specify peers that your node will maintain persistent
...
A list of available persistent peers is also available in the `#find-peers` channel in the [SERV Discord](https://discord.gg/evmos).
You can get a random 10 entries from the `peers.txt` file in the `PEERS` variable by running
the following command:

```bash
PEERS=`curl -sL https://raw.githubusercontent.com/evmos/mainnet/main/serv_53970-2/peers.txt | sort -R | head -n 10 | awk '{print $1}' | paste -s -d, -`
```

Use `sed` to include them into the configuration. You can also add them manually:

```bash
sed -i.bak -e "s/^persistent_peers *=.*/persistent_peers = \"$PEERS\"/" ~/.serv/config/config.toml
```
...
You can share your peer to posting it in the `#find-peers` channel in the [SERV Discord](https://discord.gg/evmos).
