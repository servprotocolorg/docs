...
HSM modules must support `ed25519` signatures for SERV. The [YubiHSM 2](https://www.yubico.com/products/hardware-security-module/) supports `ed25519` and can be used with this YubiKey [library](https://github.com/iqlusioninc/yubihsm.rs).
...
Learn how to set up a Key Management System for SERV with Tendermint KMS [here](./tendermint-kms).
...
```bash
~/.serv/config/priv_validator_key.json
```

Then do the following:

1. Back up the `json` file mentioned above (or backup the whole `config` folder).
2. Back up the self-delegator wallet. See [backing up wallets with the SERV Daemon](./../../protocol/concepts/key-management).
...
```bash
servnode tendermint show-address
```
...
For example, the environment variable `SERV_CHAIN_ID` will map to the command line flag `--chain-id`. Note that while
