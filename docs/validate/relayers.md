...
Before beginning, ensure you have an SERV node running in the background of the same machine that you intend to relay on.
 Follow [this guide](./../protocol/evmos-cli/single-node) to set up an SERV node if you have not already.

In this guide, we will be relaying between [SERV (channel-3) and Cosmos Hub (channel-292)](https://www.mintscan.io/evmos/relayers).
 When setting up your SERV and Cosmos full nodes, be sure to offset the ports being used in both the `app.toml` and `config.toml`
  files of the respective chains (this process will be shown below).

In this example, the default ports for SERV will be used, and the ports of the Cosmos Hub node will be manually changed.

## SERV Daemon Settings

First, set `grpc server` on port `9090` in the `app.toml` file from the `$HOME/.serv/config` directory:

```bash
vim $HOME/.serv/config/app.toml
```
...
Then, set the `pprof_laddr` to port `6060`, `rpc laddr` to port `26657`, and `prp laddr` to `26656` in the `config.toml`
 file from the `$HOME/.serv/config` directory:

```bash
vim $HOME/.serv/config/config.toml
```
...
```bash
# In this example, we will set channel-292 on the cosmoshub-4 chain settings and channel-3 on the serv_53970-2 chain settings:
[[chains]]
id = 'cosmoshub-4'
...
policy = 'allow'
list = [
   ['transfer', 'channel-292'], # serv_53970-2
]
...
hermes keys restore serv_53970-2 -m "24-word mnemonic seed"
```

Ensure this wallet has funds in both SERV and ATOM in order to pay the fees required to relay.
...
INFO ThreadId(01) [serv_53970-2] chain is healthy
```
...
```bash
hermes query packet unreceived-packets serv_53970-2 transfer channel-3
hermes query packet unreceived-acks serv_53970-2 transfer channel-3
```
...
hermes query packet commitments serv_53970-2 transfer channel-3
```
...
hermes clear packets serv_53970-2 transfer channel-3
```
...
```bash
hermes tx raw packet-recv serv_53970-2 cosmoshub-4 transfer channel-292
hermes tx raw packet-ack serv_53970-2 cosmoshub-4 transfer channel-292
hermes tx raw packet-recv cosmoshub-4 serv_53970-2 transfer channel-3
hermes tx raw packet-ack cosmoshub-4 serv_53970-2 transfer channel-3
```
