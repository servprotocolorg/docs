...
SNAP_RPC="rpc.serv.services:26657"
CHAIN_ID="serv_43970-1"
PEER=""
wget -O $HOME/genesis.json https://rpc.serv.services/genesis 
```

## Use commands below for Mainnet setup

```bash
SNAP_RPC1="https:rpc.serv.services/evmos"
SNAP_RPC="https:rpc.serv.services/evmos"
...
```bash
servnode init $moniker --chain-id $CHAIN_ID
```

Move genesis file to .serv/config folder

```bash
mv $HOME/genesis.json ~/.serv/config/
```
...
```bash
sed -i -e "s%^moniker *=.*%moniker = \"$moniker\"%; " $HOME/.serv/config/config.toml
sed -i -e "s%^indexer *=.*%indexer = \"null\"%; " $HOME/.serv/config/config.toml
sed -i -e "s%^persistent_peers *=.*%persistent_peers = \"$PEER\"%; " $HOME/.serv/config/config.toml
```
...
s|^(seeds[[:space:]]+=[[:space:]]+).*$|\1\"\"|" ~/.serv/config/config.toml
```

### Create servnode service

```bash
echo "[Unit]
Description=SERVd Node
After=network.target
#
[Service]
User=$USER
Type=simple
ExecStart=$(which servnode) start
Restart=on-failure
LimitNOFILE=65535
#
[Install]
WantedBy=multi-user.target" > $HOME/servnode.service; sudo mv $HOME/servnode.service /etc/systemd/system/
```

```bash
sudo systemctl enable servnode.service && sudo systemctl daemon-reload
```

### Run servnode

```bash
sytemctl start servnode
```

### Check logs

```bash
journalctl -u servnode -f
```
...
```bash
sed -i.bak -E "s|^(enable[[:space:]]+=[[:space:]]+).*$|\1false|" $HOME/.serv/config/config.toml
```
