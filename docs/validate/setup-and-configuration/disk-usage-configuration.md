...
speed and transaction amount. For SERV, we are talking about close to
...
I believe this was disabled by default on SERV, but listing it in any
...
will risk database corruption if the `servnode` is killed for any reason.
...
Below is the disk usage after two weeks of SERV Arsia Mons testnet. The default
