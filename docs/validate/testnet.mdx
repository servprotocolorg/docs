...
| ---------------- | --------------------------------- | -------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------- | ------- |
| `serv_43970-1`   | SERV Testnet                | [SERV](https://github.com/servprotocolorg/serv/)     | [`v12.2.3`](https://github.com/servprotocolorg/serv/releases)                                    | `Live` |
...
```bash
servnode config chain-id serv_43970-4
```
...
```bash
servnode init <your_custom_moniker> --chain-id serv_43970-4
```
...
Check the `genesis.json` file from the [`archive`](https://archive.evmos.dev/serv_43970-4/genesis.json)
 and copy it over to the `config` directory: `~/.serv/config/genesis.json`. This is
 a genesis file with the chain-id and genesis accounts balances.

```bash
sudo apt install -y unzip wget
wget -P ~/.serv/config https://archive.evmos.dev/serv_43970-4/genesis.json
```
...
Your node needs to know how to find [peers](https://docs.tendermint.com/v0.34/tendermint-core/using-tendermint.html#peers). You'll need to add healthy [seed nodes](https://docs.tendermint.com/v0.34/tendermint-core/using-tendermint.html#seed)
 to `$HOME/.serv/config/config.toml`. The [`testnets`](https://github.com/evmos/testnets) repo contains links to
  some seed nodes.
...
We can set the [`persistent_peers`](https://docs.tendermint.com/v0.34/tendermint-core/using-tendermint.html#persistent-peer) field in `~/.serv/config/config.toml` to specify peers that your node will maintain persistent
...
[SERV Discord](https://discord.gg/evmos). You can get a random 10 entries from the `peers.txt`
 file in the `PEERS` variable by running the following command:

```bash
PEERS=`curl -sL https://raw.githubusercontent.com/evmos/testnets/main/serv_43970-4/peers.txt | sort -R | head -n 10 | awk '{print $1}' | paste -s -d, -`
```
