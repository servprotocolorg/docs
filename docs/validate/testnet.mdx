...
```bash
servnode config chain-id serv_43970-1
```
...
```bash
servnode init <your_custom_moniker> --chain-id serv_43970-1
```
...
Check the `genesis.json` file from the [`archive`](https://archive.evmos.dev/serv_43970-1/genesis.json)
...
wget -P ~/.serv/config https://archive.evmos.dev/serv_43970-1/genesis.json
```
...
```bash
PEERS=`curl -sL https://raw.githubusercontent.com/evmos/testnets/main/serv_43970-1/peers.txt | sort -R | head -n 10 | awk '{print $1}' | paste -s -d, -`
```
