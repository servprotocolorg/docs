...
Users need to pay a fee to submit transactions on the SERV network.
As fees are handled differently on Ethereum and Cosmos,
it is important to understand how the SERV blockchain implements an Ethereum-type fee calculation,
...
Also, note the fees that are paid for interacting with smart contracts on SERV
...
### How are Gas and Fees Handled on SERV?

Fundamentally, SERV is a Cosmos SDK chain that enables EVM compatibility as part of a Cosmos SDK module. As a result of
...
SERV logic. For the most part, fees are collected by the `fee_collector` module, then paid out to validators and
delegators. A few key distinctions are as follows:

1. Fee Market Module

    In order to support EIP-1559 gas and fee calculation on SERV’ EVM layer, SERV tracks the gas supplied for each
    ...
    Unlike on Ethereum, the `BaseFee` on SERV is not burned, and instead is distributed to validators and delegators.
...
2. EVM Gas Refunds
    ...
    SERV refunds a fraction (at least 50% by default) of the unused gas for EVM transactions to approximate the current
    behavior on Ethereum. [Why not always 100%?](https://github.com/evmos/ethermint/issues/1085)

3. Revenue Module

    SERV developed the Revenue Module as a way to reward developers for creating useful dApps—any contract that is
    registered with SERV’ Revenue Module rewards a fraction of the transaction fee (currently 95%) from each transaction
...
SERV is an EVM-compatible chain that supports Ethereum Web3 tooling. For this reason, gas consumption must be equatable
...
For this same reason, in SERV it is possible to send transactions with `0` fees for transaction
...
(i.e `ValidateBasic`) function as well as on the custom `AnteHandler` defined by SERV.

### Gas Estimation

Ethereum provides a JSON-RPC endpoint `eth_estimateGas` to help users set up a correct gas limit in their transactions.

For that reason, a specific query API `EstimateGas` is implemented in SERV. It will apply the transaction against the
...
Let’s say a user transfers tokens from Chain A to SERV via IBC-transfer and wants to execute an SERV transaction—however,
 they don’t have any SERV tokens to cover fees. The Cosmos SDK introduced `Tips` as a solution to this issue; a user can
...
To cover transaction fees using a tip, this user can sign a transaction with a tip and no fees, then send the transaction
 to a fee relayer. The fee relayer will then cover the fee in the native currency (SERV in this case), and receive the
...
## Dealing with gas and fees with the SERV CLI

When broadcasting a transaction using the SERV CLI client, users should keep into consideration the options available.
There are three flags to consider when sending a transaction to the network:

- `--fees`: fees to pay along with transaction; eg: 10aservo. Defaults to the required fees.
- `--gas`: the gas limit to set per-transaction; the default value is 200000.
- `--gas-prices`: gas prices to determine the transaction fee (e.g. 10aservo).
