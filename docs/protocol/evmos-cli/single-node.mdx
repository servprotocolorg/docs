...
The simplest way to start a local SERV node is by using the provided helper script on the base level of the [SERV repository](https://github.com/servprotocolorg/serv/blob/main/local_node.sh),
...
:::tip
To avoid overwriting any data for a real node used in production, it was decided to store the automatically generated testing configuration at `~/.tmp-serv` instead of the default `~/.serv`.
:::

When working with the `local_node.sh` script, it is necessary to extend all `servnode` commands, that target the local test node, with the `--home ~/.tmp-serv` flag. This is mandatory, because the `home` directory cannot be stored in the `serv` configuration, which can be seen in the output below. For ease of use, it might be sensible to export this directory path as an environment variable:

```
$ export TMP=$HOME/.tmp-servnode`
$ servnode config --home $TMP
{
...
LOGLEVEL="info"
# Set dedicated home directory for the servnode instance
HOMEDIR="$HOME/.tmp-servnode"
# to trace evm
#TRACE="--trace"
TRACE=""
...
$MONIKER=testing
$KEY=dev0
$CHAINID="serv_43970-4"

# The argument $MONIKER is the custom username of your node, it should be human-readable.
servnode init $MONIKER --chain-id=$CHAINID
```
...
[configuration files](./configuration#client-configuration) are in `~/.serv` by default, but you can overwrite the
...
```bash
servnode keys add my_validator
```

Once you have created a local account, go ahead and grant it some `aservo` tokens in your chain's genesis file. Doing
so will also make sure your chain is aware of this account's existence:

```bash
servnode add-genesis-account my_validator 10000000000aservo
```
...
servnode add-genesis-account my_validator 1000000000stake,10000000000aservo
```
...
is provided, it defaults to the local node pubkey created via the `servnode init` command above.
...
servnode collect-gentxs
```

This command will add all the `gentxs` stored in `~/.serv/config/gentx` to the genesis file.
...
To run a node with the same key every time: replace `servnode keys add $KEY` in `./local_node.sh` with:

```bash
echo "your mnemonic here" | servnode keys add $KEY --recover
```

:::tip
SERV currently only supports 24 word mnemonics.
:::

You can generate a new key/mnemonic with:

```bash
servnode keys add $KEY
```

To export your SERV key as an Ethereum private key (for use with [Metamask](./../../../use/connect-your-wallet/metamask)
for example):

```bash
servnode keys unsafe-export-eth-key $KEY
```
...
SERV supports using a file or OS keyring backend for key storage. To create and use a file
stored key instead of defaulting to the OS keyring, add the flag `--keyring-backend file` to any
relevant command and the password prompt will occur through the command line. This can also be saved
as a CLI config option with:

```bash
servnode config keyring-backend file
```
...
```bash
servnode start --evm.tracer $TRACER --tracestore $TRACESTORE --pruning=nothing $TRACE --log_level $LOGLEVEL --minimum-gas-prices=0.0001aservo --json-rpc.api eth,txpool,personal,net,debug,web3
```
...
:::danger
If you are running a **validator node**, always be careful when doing `servnode unsafe-reset-all`. You should never use
...
```bash
rm $HOME/.serv/config/addrbook.json $HOME/.serv/config/genesis.json
...
```shell
rate(servnode_transactions_processed[1m])
```
