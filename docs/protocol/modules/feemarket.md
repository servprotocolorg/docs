...
You might encounter both terminologies on SERV since it builds Ethereum on top of the SDK,
...
Thus, the tip for an EIP-1559 transaction on SERV should be zero
(`MaxPriorityFeePerGas` JSON-RPC endpoint returns `0`).
Have a look at the [mempool](https://docs.serv.services/validate/setup-and-configuration/mempool) docs
...
```bash
servnode query feemarket base-fee [flags]
```

Example:

```bash
servnode query feemarket base-fee ...
```
...
```bash
servnode query feemarket block-gas [flags]
```

Example:

```bash
servnode query feemarket block-gas ...
```
...
```bash
servnode query params subspace [subspace] [key] [flags]
```

Example:

```bash
servnode query params subspace feemarket ElasticityMultiplier ...
```
