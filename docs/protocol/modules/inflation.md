...
The `x/inflation` module mints new SERV tokens and allocates them in daily
epochs according to the [SERV Token
...
The allocation of new coins incentivizes specific behaviour in the SERV
...
The SERV Token Model outlines how the SERV network is secured through a
...
issue 1 billion SERV tokens within the first 4 years.
...
daily provision (`epochProvison`) of SERV tokens is minted and allocated to staking rewards,
...
| ------------------------              | ---------------------- | ----------------------------------------------------------------------------- |
| `ParamStoreKeyMintDenom`              | string                 | `evm.DefaultEVMDenom` // “aservo”                                             |
...
the distribution from the SERV Token Model is recalculated into a distribution
that excludes team vesting. Note, that this does not change the inflation
proposed in the SERV Token Model. Each `InflationDistribution` can be
calculated like this:

```markdown
stakingRewards =servTokenModelDistribution / (1 - teamVestingDistribution)
0.5333333      = 40%                         / (1 - 25%)
```
...
Find below a list of `servnode` commands added with the `x/inflation` module. You
can obtain the full list by using the `servnode -h` command.
...
```bash
servnode query inflation period [flags]
```
...
```bash
servnode query inflation epoch-mint-provision [flags]
```
...
```bash
servnode query inflation skipped-epochs [flags]
```
...
```bash
servnode query inflation total-supply [flags]
```
...
```bash
servnode query inflation inflation-rate [flags]
```
...
```bash
servnode query inflation params [flags]
```
...
```bash
servnode tx gov submit-legacy-proposal param-change [proposal-file] [flags]
```
