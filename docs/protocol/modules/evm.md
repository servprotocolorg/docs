...
### SERV as Geth implementation

SERV contains an implementation of the [Ethereum protocol in Golang](https://geth.ethereum.org/docs/getting-started)
...
SERV can be run as a node to compute state transitions with the EVM.
SERV supports Geth's standard [Ethereum JSON-RPC APIs](https://docs.serv.services/develop/api/ethereum-json-rpc/methods)
...
The JSON-RPC method [`eth_call`](https://docs.serv.services/develop/api/ethereum-json-rpc/methods#eth-call) allows you
...
The SERV implementation is similar and makes use of the gRPC query client which is included in the Cosmos SDK:
...
The implementation of this interface is what makes SERV EVM compatible.
...
SERV adapted the [go-ethereum journal implementation](https://github.com/ethereum/go-ethereum/blob/master/core/state/journal.go#L39)
...
SERV uses the Cosmos `KVStore` (key-value store) and Cosmos SDK `Keeper` to facilitate state transitions.
...
Its main difference is that the one on SERV uses a custom `Storage` type
...
Addresses use the **[EIP55](https://eips.ethereum.org/EIPS/eip-55)** hex **[format](https://docs.serv.services/protocol/concepts/accounts#address-formats-for-clients)**
on `genesis.json`.
...
### Use Case: Call Native ERC20 Module on SERV

Here is an example taken from the SERV [erc20 module](erc20.md)
...
In the case of SERV, the default denomination is the **attoserv**.
In terms of precision, `SERV` and `ETH` share the same value,
*i.e.* `1 SERV = 10^18 attoserv` and `1 ETH = 10^18 wei`.

:::tip
Note: SDK applications that want to import the EVM module as a dependency
will need to set their own `evm_denom` (i.e not `"aservo"`).
:::
...
Find below a list of `servnode` commands added with the `x/evm` module.
...
```bash
servnode query evm code ADDRESS [flags]
```

```bash
# Example
$ servnode query evm code 0x7bf7b17da59880d9bcca24915679668db75f9397
...
```bash
servnode query evm storage ADDRESS KEY [flags]
```

```bash
# Example
$ servnode query evm storage 0x0f54f47bf9b8e317b214ccd6a7c3e38b893cd7f0 0 --height 0
...
```bash
servnode tx evm raw TX_HEX [flags]
```

```bash
# Example
$ servnode tx evm raw 0xf9ff74c86aefeb5f6019d77280bbb44fb695b4d45cfe97e6eed7acd62905f4a85034d5c68ed25a2e7a8eeb9baf1b84
...
For an overview on the JSON-RPC methods and namespaces supported on SERV,
please refer to [https://docs.serv.services/develop/api/ethereum-json-rpc/methodsl](https://docs.serv.services/develop/api/ethereum-json-rpc/methods)
