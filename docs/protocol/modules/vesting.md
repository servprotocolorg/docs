...
This document specifies the internal `x/vesting` module of the SERV Hub.
...
For SERV, the `ClawbackVestingAccount` is used to allocate tokens to core team members and advisors
...
that spend SERV using the `x/evm` module.
However, the account can perform Ethereum transactions that don't spend SERV tokens.
...
| Token Status            | Transfer | Delegate | Vote | Eth Txs that spend SERV\*\* | Eth Txs that don't spend SERV (amount = 0)\*\* |
| ----------------------- | :------: | :------: | :--: | :--------------------------: | :---------------------------------------------: |
| `locked` & `unvested`   |    ❌    |    ❌    |  ❌  |              ❌              |                       ✅                        |
| `locked` & `vested`     |    ❌    |    ✅    |  ✅  |              ❌              |                       ✅                        |
| `unlocked` & `unvested` |    ❌    |    ❌    |  ❌  |              ❌              |                       ✅                        |
...
\*\*EVM transactions only fail if they involve sending locked or unvested SERV tokens,
e.g. send SERV to EOA or Smart Contract (fails if amount > 0 ).
...
```bash
vestcalc --write --start=2022-01-01 --coins=200000000000000000000000aservo --months=48 --cliffs=2023-01-01
```
...
A user can query the SERV `x/vesting` module using the CLI, gRPC, or REST.

### CLI

Find below a list of `servnode` commands added with the `x/vesting` module.
...
```go
servnode add-genesis-account ADDRESS_OR_KEY_NAME COIN... [flags]
```
...
```go
servnode query vesting balances ADDRESS [flags]
```
...
```go
servnode tx vesting create-clawback-vesting-account TO_ADDRESS [flags]
```
...
```go
servnode tx vesting clawback ADDRESS [flags]
```
...
```go
servnode tx vesting update-vesting-funder VESTING_ACCOUNT_ADDRESS NEW_FUNDER_ADDRESS --from=FUNDER_ADDRESS [flags]
```
...
```go
servnode tx vesting convert VESTING_ACCOUNT_ADDRESS [flags]
```
