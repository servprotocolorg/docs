...
Make sure to look at [SERV Governance](https://academy.evmos.org/articles/advanced/governance/),
and specifically the [best practices](https://academy.evmos.org/articles/advanced/governance/best-practices).
:::

## Abstract

This document specifies the internal `x/erc20` module of the SERV Hub.

The `x/erc20` module enables the SERV Hub to support a trustless, on-chain bidirectional internal conversion of tokens
between SERV' EVM and Cosmos runtimes, specifically the `x/evm` and `x/bank` modules.
This allows token holders on SERV to instantaneously convert their native Cosmos `sdk.Coins`
...
This conversion functionality is fully governed by native SERV token holders
...
with the difference, that it not only applies to gas tokens (like SERV),
but to all Cosmos Coins (IBC vouchers, staking and gov coins, etc.) as well.

With the `x/erc20` users on SERV can
...
- transfer existing tokens on Ethereum and other EVM-based chains to SERV
...
It can be either the native staking/gas denomination (e.g. SERV, ATOM, etc)
...
Holders of native Cosmos coins and IBC vouchers on the SERV chain can convert their Coin into ERC20 Tokens,
which can then be used in SERV EVM, by creating a `ConvertCoin` Tx.
Vice versa, the `ConvertERC20` Tx allows holders of ERC20 tokens on the SERV chain
...
please review the x/erc20 audit, section `IF-SERV-06: IERC20 Contracts may execute arbitrary code`.
...
Note that the native SERV coin cannot be registered,
as any coin including "evm" in its denomination cannot be registered.
Instead, the SERV token can be converted by Nomand's wrapped SERV (WSERV) contract.

1. User submits a `RegisterCoinProposal`
2. Validators of the SERV Hub vote on the proposal using `MsgVote` and proposal passes
3. If Cosmos coin or IBC voucher exist on the bank module supply,
   create the [ERC20 token contract](https://github.com/servprotocolorg/serv/blob/main/contracts/ERC20MinterBurnerDecimals.sol)
...
2. Validators of the SERV chain vote on the proposal using `MsgVote` and proposal passes
...
- There shouldn't exist any native Cosmos Coin ERC20 Contract (eg SERV, Atom,
...
Find below a list of  `servnode` commands added with the  `x/erc20` module.
You can obtain the full list by using the `servnode -h` command.
A CLI command can look like this:

```bash
servnode query erc20 params
```
...
```bash
servnode tx gov submit-legacy-proposal register-coin METADATA_FILE [flags]
```
...
```bash
servnode tx gov submit-legacy-proposal register-erc20 ERC20_ADDRESS... [flags]
```
...
```bash
servnode tx gov submit-legacy-proposal toggle-token-conversion TOKEN [flags]
```
...
```bash
servnode tx gov submit-legacy-proposal param-change PROPOSAL_FILE [flags]
```
