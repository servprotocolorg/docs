...
This document specifies the internal `x/incentives` module of the SERV Hub.

The `x/incentives` module is part of the SERV tokenomics and aims
to increase the growth of the network by distributing rewards
to users who interact with incentivized smart contracts.
The rewards drive users to interact with applications on SERV and reinvest their rewards in more services in the network.
...
The incentives functionality is fully governed by native SERV token holders
who manage the registration of `Incentives`,
so that native SERV token holders decide which application should be part of the usage incentives.
...
Say, there are several incentives that have registered an allocation for the SERV coin
and the allocation meter for SERV is at 97%.
Then a new incentve proposal can only include an SERV allocation at up to 3%,
claiming the last remaining allocation capacity from the SERV rewards in the inflation pool.
...
3. Create incentive for the contract with a `TotalGas = 0` and set its `startTime` to `ctx.Blocktime`
   if the following conditions are met:
    ...
    3. Balance in the inflation pool is > 0 for each allocation denom except for the mint denomination.
       We know that the amount of the minting denom (e.g. SERV) will be added to every block
...
2. An `epoch` begins and `rewards` (SERV and other denoms) that are minted on every block for inflation
...
Find below a list of `servnode` commands added with the `x/incentives` module.
...
```go
servnode query incentives incentives [flags]
```
...
```go
servnode query incentives incentive CONTRACT_ADDRESS [flags]
```
...
```bash
servnode query incentives gas-meters CONTRACT_ADDRESS [flags]
```
...
```go
servnode query incentives gas-meter CONTRACT_ADDRESS PARTICIPANT_ADDRESS [flags]
```
...
```bash
servnode query incentives params [flags]
```
...
```bash
servnode tx gov submit-legacy-proposal register-incentive CONTRACT_ADDRESS ALLOCATION EPOCHS [flags]
```
...
```bash
servnode tx gov submit-legacy-proposal cancel-incentive CONTRACT_ADDRESS [flags]
```
